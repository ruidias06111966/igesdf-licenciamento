...
const normativaSchema = z.object({
  ...
  tipo: z.enum(["rdc","portaria","normativa","instrucao_normativa","lei","decreto","resolucao","processo_sei","outro"]),
...
export const upsertNormativa = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    delete clean.id;
    const { data: ins, error } = await context.supabase.from("normativas").insert(clean).select("id").single();
    if (error) throw error;
    return { id: ins.id };
  });
...
export const deleteNormativa = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ id: z.string().uuid(), storage_path: z.string().nullable().optional() }).parse(input))
  .handler(async ({ data, context }) => {
    if (data.storage_path) await context.supabase.storage.from("licencas-docs").remove([data.storage_path]);
